...
dgnDscrPtr getDgn(string sign) {
    u i = 0;
    while (i < DGNCOUNT && stringEquals(sign, sstr(DGNS[i]->id)))
        i++;
    return i < DGNCOUNT ? DGNS[i] : NULL;
}
...
bool isGOP(context* c, string code, par* pars, OP* op) {
    ...
    for (u i = 0; i < OPCOUNT; i++)
        if (stringEquals(sstr(OPS[i].code), code) || stringEquals(sstr(OPS[i].alias), code)) {
            *op = i;
            if (OPS[i].arg == FNONE)
                cAddDgnLoc(c, &ENOPAR, pars[0].loc, cptr(code));
            else {
                if (pars[0].kind == KNONE)
                    cAddDgn(c, &EMISSINGSYNTAX, "parameter for opcode");
                else
                    cAddDgnLoc(c, &EWRONGPAR, pars[0].loc, cptr(code));
            }
            return (OPS[i].flags & FGENERIC) != 0;
        }
    *op = OPCOUNT;
...
bool addFile(context* c, string path, loc loc) {
    if (fileExists(path)) {
        ...
        if (res)
            ...
            cAddDgn(c, &MMULTIFILE, cptr(path));
        return res;
    } else {
        ...
        if (fileExists(tmp)) {
            ...
            cAddDgnLoc(c, &EFILENOTEXIST, loc, cptr(path));
            return false;
        }
    }
}
    ...
    for (u i = 0; i < atts.len; i++) {
        if (ATTRIBUTES[atts.items[i].kind].flags & FSINGLE) {
            if (uListContains(ul, atts.items[i].kind))
                cAddDgnEmptyLoc(c, &ESINGLEATT, atts.items[i].loc);
            else
                uListAdd(&ul, atts.items[i].kind);
        }
        if ((ATTRIBUTES[atts.items[i].kind].trgt & t) == 0)
            cAddDgnLoc(c, &EWRONGTARGET, atts.items[i].loc, ATTRIBUTES[atts.items[i].kind].name);
    ...
    for (u i = 0 ; i < vars.len; i++)
        if (vars.items[i].flags & FDEFINED) {
            if ((c->typs.items[vars.items[i].type.i].flags & FDEFINED) == 0)
                cAddDgnLoc(c, &EDEFNOTFOUND, vars.items[i].type.loc, cptr(c->typs.items[vars.items[i].type.i].name.sign));
            if (lastDef == vars.len)
            ...
            if ((t & (FGLB | FFLD)) != 0 && emitNoRef(c, as(def, &vars.items[i])) || (t & (FGLB | FFLD)) == 0 && (vars.items[i].flags & FREFERENCED) == 0)
                cAddDgnLoc(c, &MNOTREFERENCED, vars.items[i].name.loc, cptr(vars.items[i].name.sign));
    ...
    for (u i = 0; i < c->typs.len; i++)
        if (c->typs.items[i].flags & FDEFINED) {
            ...
            if (emitNoRef(c, as(def, &c->typs.items[i])))
                cAddDgnLoc(c, &MNOTREFERENCED, c->typs.items[i].name.loc, cptr(c->typs.items[i].name.sign));
...
static void linkTypList(context* c, list(ref) typs) {
    for (u i = 0; i < typs.len; i++)
        if ((c->typs.items[typs.items[i].i].flags & FDEFINED) == 0)
            cAddDgnLoc(c, &EDEFNOTFOUND, typs.items[i].loc, cptr(c->typs.items[typs.items[i].i].name.sign));
}
static void linkParam(context* c, opc* o, u f) {
    if (as(popc, o)->par.kind == KFUN && (c->funs.items[as(popc, o)->par.val.r.i].flags & FDEFINED) == 0)
        cAddDgnLoc(c, &EDEFNOTFOUND, as(popc, o)->par.val.r.loc, cptr(c->funs.items[as(popc, o)->par.val.r.i].name.sign));
        ...
        if ((c->typs.items[as(popc, o)->par.val.r.i].flags & FDEFINED) == 0)
            ...
            cAddDgnLoc(c, &EDEFNOTFOUND, as(popc, o)->par2.val.r.loc, cptr(c->typs.items[as(popc, o)->par.val.r.i].flds.items[as(popc, o)->par2.val.r.i].name.sign));
    } else if (as(popc, o)->par.kind == KGLB && (c->glbs.items[as(popc, o)->par.val.r.i].flags & FDEFINED) == 0)
        cAddDgnLoc(c, &EDEFNOTFOUND, as(popc, o)->par.val.r.loc, cptr(c->glbs.items[as(popc, o)->par.val.r.i].name.sign));
    else if (as(popc, o)->par.kind == KARG && (c->funs.items[f].args.items[as(popc, o)->par.val.r.i].flags & FDEFINED) == 0)
        cAddDgnLoc(c, &EDEFNOTFOUND, as(popc, o)->par.val.r.loc,
...
void linkBody(context* c, body* b, u f, i64* s) {
    ...
    for (u i = 0; i < b->ops.len; i++) {
        ...
        if (*s < 0){
            cAddDgnEmptyLoc(c, &ESTACKLOW, b->ops.items[i]->loc);
            *s = 0;
        }
        if (OPS[b->ops.items[i]->op].flags & FARGCRETC)
        ...
        if ((b->ops.items[i]->op == OPRET || b->ops.items[i]->op == OPTHROW || b->ops.items[i]->op == OPEXIT) && i != b->ops.len - 1)
            cAddDgnEmptyLoc(c, &WUNREACHCODE, b->ops.items[i + 1]->loc);
    }
    b->retc = *s - b->retc;
}
    ...
    for (u i = 0; i < c->funs.len; i++)
        if (c->funs.items[i].flags & FDEFINED) {
            ...
            if (c->funs.items[i].body.flags & FPARSED) {
                ...
                if (b > 0)
                    cAddDgnLoc(c, &WSTACKHIGH, c->funs.items[i].name.loc, cptr(utos(b)));
                if ((c->funs.items[i].body.flags & FSTOPS) == 0)
                    cAddDgnEmptyLoc(c, &ENOTSTOPS, c->funs.items[i].name.loc);
            }
            c->funs.items[i].name.csign = getCsign(c->funs.items[i].name.sign);
            if (hasAtt(c->funs.items[i].attrs, ATTMAIN, NULL)) {
                if (c->flags & FHASMAIN)
                    cAddDgnEmptyLoc(c, &EMULTIMAIN, c->funs.items[i].name.loc);
                else {
                    c->flags |= FHASMAIN;
                    c->main = i;
                }
            } else if (emitNoRef(c, as(def, &c->funs.items[i])))
                cAddDgnLoc(c, &MNOTREFERENCED, c->funs.items[i].name.loc, cptr(c->funs.items[i].name.sign));
...
void link(context* c) {
    ...
    if (c->flags & FHASMAIN) {
        if (c->funs.items[c->main].args.len != 0)
            cAddDgnLoc(c, &EWRONGNUMOFARGS, c->funs.items[c->main].name.loc, cptr(c->funs.items[c->main].name.sign));
    } else
        cAddDgn(c, &EMISSINGSYNTAX, "function with attribute 'main'");
    c->flags |= FLINKED;
}
